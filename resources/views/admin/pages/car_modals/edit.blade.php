...
<div class="row">
    <div class="col-8 mx-auto mt-3">
        <div class="card">
            <div class="card-body">
                <form action="{{ route('admin.car_modals.update',$car_modal->id) }}" method="post" enctype="multipart/form-data">
                                ...
                                @foreach (config('laravellocalization.supportedLocales') as $key=>$value)
                                    <div class="tab-pane fade {{ ($loop->index == 0)? 'show active': '' }}" id="v-pills-{{$key}}" role="tabpanel" aria-labelledby="v-pills-{{$key}}-tab">
                                        <div class="form-floating mb-3">
                                            ...
                                            value="{{ old('name',$car_modal->translate($key)->name ?? '') }}">
                    ...
                    <div class="form-floating mb-3">
                        <select name="parent_id" class="form-select">
                            ...
                            @foreach ($items as $item)
                                <option @if ($car_modal->parent_id == $item->id)
                                    selected

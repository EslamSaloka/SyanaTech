...
<div style=" margin-bottom: 14px; position: relative; text-align: @if(\App::getLocale() == 'ar') left @else right @endif; ">
    <a type="button" class="btn btn-primary"  href="{{route('admin.colors.create')}}">@lang('Create new color')</a>
</div>
                    ...
                    <table class="table project-list-table table-nowrap align-middle table-borderless">
                        ...
                        <tbody>
                            @foreach ($lists as $list)
                                <tr>
                                    <td>
                                        <a href="{{ route('admin.colors.edit',$list->id) }}">
                                            {{ $list->name ?? '' }}
                                        </a>
                                    </td>
                                    <td>
                                    ...
                                    <td style="display: inline-flex;">
                                        <a style="@if(\App::getLocale() == 'ar') margin-left: 5px; @else margin-right: 5px; @endif"
                                            ...
                                            href="{{ route('admin.colors.edit',$list->id) }}">
                                            <i class="bx bx-pencil"></i>
                                        </a>
                                        {!! action_table_delete(route('admin.colors.destroy',$list->id),$list->id) !!}                                        
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>

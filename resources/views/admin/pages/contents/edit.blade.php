            ...
            <div class="card-body">
                <form action="{{ route('admin.contents.update',$content->id) }}" method="post" enctype="multipart/form-data">
                                ...
                                @foreach (config('laravellocalization.supportedLocales') as $key=>$value)
                                    <div class="tab-pane fade {{ ($loop->index == 0)? 'show active': '' }}" id="v-pills-{{$key}}" role="tabpanel" aria-labelledby="v-pills-{{$key}}-tab">
                                        <div class="form-floating mb-3">
                                            ...
                                            value="{{ old($key.'.title',$content->translate($key)->title ?? '') }}">
                                        ...
                                        <div class="form-floating mb-3">
                                            ...
                                            placeholder="{{ __('Enter Description') }}">
                                                {{ old($key.'.description',$content->translate($key)->description ?? '') }}
                                            </textarea>
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-lg-1">
                            <img src="{{ display_image_by_model($content,'image') }}" alt="" class="rounded-circle header-profile-user">
                        </div>
                    </div>
